"""
MEMEBOT 2D Character Renderer - Accessories Module Part 2
Outfit Details, Hats, Masks, Glasses, Shirts, Pants, Shoes, Hair, Ears
Part 2 of 2
"""
...
def draw_hat(renderer, draw, c, cx, head_y, head_shift, hat_type):
    ...
    if hat_type == "Top Hat":
        ...
        angle = renderer.dance_timer * 0.3; prop_len = 18
        for blade in range(2):
            blade_angle = angle + blade * math.pi
            dx = int(prop_len * math.cos(blade_angle)); dy = int(prop_len * math.sin(blade_angle))
...
def draw_mask(draw, c, cx, head_y, head_shift, mask_type):
    ...
    if mask_type == "Batman":
        ...
        draw.polygon([(cx-3, mask_y+25), (cx+3, mask_y+25), (cx+5, mask_y+35), (cx-5, mask_y+35)], fill=(20,20,40,220), outline=(10,10,20,255), width=1)
        ...
        for i in range(5):
            for j in range(4):
                sx = cx - 16 + j * 8 + (i % 2) * 4; sy = mask_y + 3 + i * 5
...
def draw_shirt(draw, c, cx, hip_y, chest_y, shoulder_y, shirt_type):
    ...
    if shirt_type == "Suit":
        ...
        for i in range(4):
            for j in range(4):
                ix = cx - 18 + i * 12; iy = shoulder_y + 10 + j * 12
                if iy < hip_y - 5:
                    draw.ellipse([ix-2, iy-2, ix+2, iy+2], fill=(0, 0, 0, 100))
    elif shirt_type == "Hawaiian":
        draw.rectangle([cx-30, shoulder_y+5, cx+30, hip_y-5], fill=color, outline=outline, width=2)
        for i in range(6):
            fx = cx - 22 + i * 8; fy = chest_y - 8 + i * 5
            for j in range(5):
                petal_angle = j * (2*math.pi/5)
                px = fx + int(4 * math.cos(petal_angle)); py = fy + int(4 * math.sin(petal_angle))
        ...
        for i in range(8):
            for j in range(6):
                rx = cx - 25 + j * 8 + (i % 2) * 4; ry = shoulder_y + 8 + i * 8
                if ry < hip_y - 5:
...
def draw_pants(draw, c, cx, hip_y, ground_y, leg_swing, pants_type):
    ...
    if pants_type == "Skirt":
        draw.polygon([(cx-20, hip_y-5), (cx+20, hip_y-5), (cx+28, hip_y+35), (cx-28, hip_y+35)], fill=color, outline=outline, width=2)
        for i in range(6):
            px = cx - 15 + i * 6
        ...
        for side, hx in [(-1,cx-20),(1,cx+20)]:
            draw.rectangle([hx-10, hip_y-5, hx+10, hip_y+30], fill=color, outline=outline, width=2)
    else:
        for side, hx in [(-1,cx-20),(1,cx+20)]:
            draw.rectangle([hx-10, hip_y-5, hx+10, ground_y-70], fill=color, outline=outline, width=2)
            draw.rectangle([hx-6, hip_y-3, hx-2, ground_y-72], fill=light_color)
            draw.line([hx-9, ground_y-75, hx+9, ground_y-75], fill=dark_color, width=2)
...
def draw_shoes(draw, c, cx, ground_y, leg_swing, shoes_type):
    ...
    for fx in [cx-14+leg_swing, cx+14-leg_swing]:
        if shoes_type == "Sneakers":
            ...
            for i in range(4):
                lx = fx - 5 + i * 3; ly = ground_y - 70 + i * 4
...
def draw_hair_style(renderer, draw, c, hair_cfg, cx, adj_head_y, head_shift, head_size_mult, width_mult):
    ...
    if hair_style == "bald":
        ...
        if hair_bangs:
            draw.ellipse([cx - int(44 * hair_vol) + head_shift, adj_head_y - 6, cx + int(44 * hair_vol) + head_shift, adj_head_y + int(15 * hair_len)], fill=hair_color, outline=c["hair_outline"], width=1)
            draw.ellipse([cx - int(30 * hair_vol) + head_shift, adj_head_y - 3, cx + int(10 * hair_vol) + head_shift, adj_head_y + int(8 * hair_len)], fill=hair_light)
    elif hair_style == "long":
        draw.ellipse([cx - int(46 * hair_vol) + head_shift, adj_head_y - 10, cx + int(46 * hair_vol) + head_shift, adj_head_y + int(50 * hair_len)], fill=hair_color, outline=c["hair_outline"], width=1)
        draw.rectangle([cx - int(30 * hair_vol) + head_shift, adj_head_y + int(30 * hair_len), cx + int(30 * hair_vol) + head_shift, adj_head_y + int(90 * hair_len)], fill=hair_color, outline=c["hair_outline"])
        for i in range(8):
            strand_x = cx - int(20 * hair_vol) + i * int(5 * hair_vol) + head_shift
            draw.line([strand_x, adj_head_y + int(20 * hair_len), strand_x + 2, adj_head_y + int(80 * hair_len)], fill=hair_dark, width=1)
        draw.rectangle([cx - int(15 * hair_vol) + head_shift, adj_head_y + int(35 * hair_len), cx - int(5 * hair_vol) + head_shift, adj_head_y + int(75 * hair_len)], fill=hair_light)
    elif hair_style == "mohawk":
        draw.rectangle([cx - int(8 * hair_vol) + head_shift, adj_head_y - int(50 * hair_len), cx + int(8 * hair_vol) + head_shift, adj_head_y], fill=hair_color, outline=c["hair_outline"])
        for i in range(6):
            spike_x = cx - int(5 * hair_vol) + i * int(2 * hair_vol) + head_shift
            spike_h = int(15 * hair_len) + (i % 3) * int(10 * hair_len)
            draw.line([spike_x, adj_head_y - int(40 * hair_len), spike_x, adj_head_y - int(40 * hair_len) - spike_h], fill=hair_color, width=3)
        for i in range(5):
        ...
        for i in range(3):
            offset = i * 2; alpha = 255 - i * 30
            layer_color = tuple(list(hair_color[:3]) + [alpha])
            draw.ellipse([cx - int(55 * hair_vol) + head_shift - offset, adj_head_y - int(30 * hair_len) - offset, cx + int(55 * hair_vol) + head_shift + offset, adj_head_y + int(25 * hair_len) + offset], fill=layer_color, outline=c["hair_outline"], width=2)
        for i in range(20):
        ...
        draw.ellipse([cx - int(42 * hair_vol) + head_shift, adj_head_y - 6, cx + int(42 * hair_vol) + head_shift, adj_head_y + int(20 * hair_len)], fill=hair_color, outline=c["hair_outline"], width=1)
        draw.line([cx + int(20 * hair_vol) + head_shift, adj_head_y + int(10 * hair_len), cx + int(25 * hair_vol) + head_shift, adj_head_y + int(60 * hair_len)], fill=hair_color, width=int(12 * hair_vol))
        draw.line([cx + int(18 * hair_vol) + head_shift, adj_head_y + int(12 * hair_len), cx + int(20 * hair_vol) + head_shift, adj_head_y + int(55 * hair_len)], fill=hair_light, width=int(4 * hair_vol))
        draw.ellipse([cx + int(17 * hair_vol) + head_shift, adj_head_y + int(8 * hair_len), cx + int(23 * hair_vol) + head_shift, adj_head_y + int(14 * hair_len)], fill=(255, 100, 150, 255), outline=(200, 70, 100, 255))
        draw.ellipse([cx + int(18 * hair_vol) + head_shift, adj_head_y + int(55 * hair_len), cx + int(32 * hair_vol) + head_shift, adj_head_y + int(70 * hair_len)], fill=hair_color, outline=c["hair_outline"])
    elif hair_style == "buzzcut":
        draw.ellipse([cx - int(40 * hair_vol) + head_shift, adj_head_y - 4, cx + int(40 * hair_vol) + head_shift, adj_head_y + int(5 * hair_len)], fill=hair_color, outline=c["hair_outline"], width=1)
        for i in range(15):
        ...
        draw.ellipse([cx - int(44 * hair_vol) + head_shift, adj_head_y - 6, cx + int(44 * hair_vol) + head_shift, adj_head_y + int(30 * hair_len)], fill=hair_color, outline=c["hair_outline"], width=1)
        draw.ellipse([cx - int(25 * hair_vol) + head_shift, adj_head_y - 3, cx + int(10 * hair_vol) + head_shift, adj_head_y + int(15 * hair_len)], fill=hair_light)
...
def draw_ear_style(renderer, draw, c, body_color, body_outline_color, ear_style, ear_size, cx, ear_y, head_shift):
    ...
    if ear_style == "pointy":
        for es in [-1, 1]:
            tip_x = cx - int(18 * ear_size) * es + head_shift; tip_y = ear_y - int(60 * ear_size)
            draw.line([cx - int(24 * ear_size) * es + head_shift, ear_y, tip_x, tip_y], fill=body_outline_color, width=int(14 * ear_size))
            draw.line([cx - int(24 * ear_size) * es + head_shift, ear_y, tip_x, tip_y], fill=body_color, width=int(9 * ear_size))
            draw.line([cx - int(22 * ear_size) * es + head_shift, ear_y-2, tip_x-2, tip_y-2], fill=ear_light, width=int(4 * ear_size))
            draw.polygon([(tip_x-3, tip_y), (tip_x+3, tip_y), (tip_x, tip_y-10)], fill=body_color, outline=body_outline_color)
    elif ear_style == "cat":
        for es in [-1, 1]:
            tip_x = cx - int(18 * ear_size) * es + head_shift; tip_y = ear_y - int(55 * ear_size)
            draw.polygon([(cx - int(28 * ear_size) * es + head_shift, ear_y), (tip_x, tip_y), (cx - int(10 * ear_size) * es + head_shift, ear_y)], fill=body_color, outline=body_outline_color, width=2)
            draw.polygon([(cx - int(24 * ear_size) * es + head_shift, ear_y - 3), (tip_x, tip_y + 10), (cx - int(14 * ear_size) * es + head_shift, ear_y - 3)], fill=c["ears_inner"])
            draw.polygon([(cx - int(22 * ear_size) * es + head_shift, ear_y - 1), (tip_x-2, tip_y + 6), (cx - int(16 * ear_size) * es + head_shift, ear_y - 1)], fill=(255, 200, 220, 150))
    elif ear_style == "elf":
        for es in [-1, 1]:
            tip_x = cx - int(15 * ear_size) * es + head_shift; tip_y = ear_y - int(70 * ear_size)
            draw.line([cx - int(22 * ear_size) * es + head_shift, ear_y, tip_x, tip_y], fill=c["skin_outline"], width=int(12 * ear_size))
            draw.line([cx - int(22 * ear_size) * es + head_shift, ear_y, tip_x, tip_y], fill=c["skin"], width=int(7 * ear_size))
            draw.line([cx - int(21 * ear_size) * es + head_shift, ear_y-1, tip_x-1, tip_y-1], fill=(255, 240, 210, 150), width=int(2 * ear_size))
        ...
        for es in [-1, 1]:
            tip_x = cx - int(15 * ear_size) * es + head_shift; tip_y = ear_y - int(65 * ear_size)
            draw.polygon([(cx - int(22 * ear_size) * es + head_shift, ear_y), (tip_x, tip_y), (cx - int(8 * ear_size) * es + head_shift, ear_y + 5)], fill=(180, 30, 30, 255), outline=(120, 15, 15, 255), width=2)
            draw.polygon([(cx - int(20 * ear_size) * es + head_shift, ear_y), (tip_x-1, tip_y + 5), (cx - int(10 * ear_size) * es + head_shift, ear_y + 3)], fill=(255, 60, 60, 100))
    elif ear_style == "mouse":
        for es in [-1, 1]:
            draw.ellipse([cx - int(35 * ear_size) * es + head_shift, ear_y - int(50 * ear_size), cx - int(10 * ear_size) * es + head_shift, ear_y + int(5 * ear_size)], fill=c["skin"], outline=c["skin_outline"], width=2)
            draw.ellipse([cx - int(30 * ear_size) * es + head_shift, ear_y - int(42 * ear_size), cx - int(15 * ear_size) * es + head_shift, ear_y - int(2 * ear_size)], fill=c["ears_inner"])
            for v in range(3):
        ...
        for es in [-1, 1]:
            tip_x = cx - int(18 * ear_size) * es + head_shift; tip_y = ear_y - int(50 * ear_size)
            draw.line([cx - int(24 * ear_size) * es + head_shift, ear_y, tip_x, tip_y], fill=body_outline_color, width=int(14 * ear_size))
            draw.line([cx - int(24 * ear_size) * es + head_shift, ear_y, tip_x, tip_y], fill=body_color, width=int(9 * ear_size))
